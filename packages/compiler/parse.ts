import { camelize } from "@flib/utils";
import { NodeTypes, createRoot, ElementTypes } from "./ast";
import { isVoidTag, isNativeTag } from "./index";

type Options = {
  delimiters: string[];
  isNativeTag: (val: string) => boolean;
  isVoidTag: (val: string) => boolean;
};
...
function parseTag(context: Context): Node {
  // Tag open.
  const match = /^<\/?([a-z][^\t\r\n\f />]*)/i.exec(
    context.source
  ) as RegExpExecArray;
...
function isComponent(tag: string, context: Context) {
  const { options } = context;
  return !options.isNativeTag(tag);
}

function advanceSpaces(context: Context) {
...
function parseAttribute(context: Context) {
  // Name.
...
function parseAttributeValue(context: Context) {
  // 不考虑没有引号的情况
